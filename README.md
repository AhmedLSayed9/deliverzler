...
An open-source Delivery App for Restaurants built on Flutter using Domain-Driven Design (DDD) and Riverpod Framework with Firebase as backend, Navigation 2.0 (GoRouter), Google Maps, Live Tracking, Local Notifications, FCM Notifications, Cloud Functions, Flutter Hooks, Freezed, RxDart, FpDart and more!
...
_Note_: This is my current approach and architecture. In the other branches, a Clean Architecture 
approach is also utilized if you're interested:
- clean-arch branch: Clean Architecture approach using Riverpod 2.0 with code generation.
- clean-arch-no-code-gen branch: Clean Architecture approach using Riverpod 2.0 without code generation.
...
* Enhanced DDD architecture by relying on Riverpod to catch exceptions and auto convert it to AsyncError instead of the traditional Result/Either pattern, which reduce lots of boilerplate code.
